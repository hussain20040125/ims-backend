...
class TransactionService {
  static {
    __name(this, "TransactionService");
  }
  // --- Stock Update Helper (Private/Internal) ---
  static async updateStock(type, sku, itemName, qty, unit, category, session) {
  ...
  // --- Inward Endpoints ---
  static async createInward(data, createdBy) {
    ...
    try {
      ...
      const inward = await Inward.create([inwardData]);
      ...
      checkAndFireLowStockWebhook(data.items.map((i) => i.sku)).catch((err) => logger.error(err));
      return inward[0];
    } catch (error) {
  ...
  static async updateInward(id, data, updatedBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
    ...
    session.startTransaction();
    try {
      const oldItem = await Inward.findOne({ id });
      if (!oldItem) throw new Error("Inward item not found");
      const newData = { ...data };
      delete newData._id;
      for (const item of oldItem.items) {
        await this.updateStock("Inward", item.sku, item.itemName, -item.qty, item.unit || "", oldItem.category || "General", session);
      }
      for (const item of newData.items) {
        await this.updateStock("Inward", item.sku, item.itemName, item.qty, item.unit || "", newData.category || "General", session);
      }
      const updated = await Inward.findOneAndUpdate({ id }, newData, { returnDocument: 'after', session });
      ...
      checkAndFireLowStockWebhook(newData.items.map((i) => i.sku)).catch((err) => logger.error(err));
      return updated;
    } catch (error) {
      await session.abortTransaction();
      throw error;
  ...
  static async deleteInward(id, deletedBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
    ...
    session.startTransaction();
    try {
      const item = await Inward.findOne({ id });
      if (item) {
        for (const it of item.items) {
          await this.updateStock("Inward", it.sku, it.itemName, -it.qty, it.unit || "", item.category || "General", session);
        }
        await Inward.findOneAndDelete({ id });
  ...
  // --- Outward Endpoints ---
  static async createOutward(data, createdBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
  ...
  // --- Outward Deletion ---
  static async deleteOutward(id, deletedBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
    ...
    try {
      ...
      await Transaction.findOneAndDelete({ id });
      await Outward.findOneAndDelete({ id });
      await session.commitTransaction();
      return true;
    } catch (error) {
      await session.abortTransaction();
  ...
  static async createInwardReturn(data, createdBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
  ...
  static async updateInwardReturn(id, data, updatedBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
    ...
    session.startTransaction();
    try {
      const oldItem = await InwardReturn.findOne({ id });
      ...
      const item = await InwardReturn.findOneAndUpdate({ id }, data, { returnDocument: 'after', session });
      await Transaction.findOneAndUpdate({ id }, { ...data, type: "Inward Return" });
      await session.commitTransaction();
  ...
  static async deleteInwardReturn(id, deletedBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
    ...
    session.startTransaction();
    try {
      const item = await InwardReturn.findOne({ id });
      if (item) {
        for (const it of item.items) {
          await this.updateStock("Inward Return", it.sku, it.itemName, -it.qty, it.unit, "General", session);
        }
        await InwardReturn.findOneAndDelete({ id });
        await Transaction.findOneAndDelete({ id });
  ...
  static async createOutwardReturn(data, createdBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
  ...
  static async updateOutwardReturn(id, data, updatedBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
    ...
    session.startTransaction();
    try {
      const oldItem = await OutwardReturn.findOne({ id });
      if (!oldItem) throw new Error("Outward Return not found");
      const item = await OutwardReturn.findOneAndUpdate({ id }, data, { returnDocument: 'after', session });
      await Transaction.findOneAndUpdate({ id }, { ...data, type: "Outward Return" });
      await session.commitTransaction();
  ...
  static async deleteOutwardReturn(id, deletedBy) {
    const session = { startTransaction: /* @__PURE__ */ __name(() => {
    }, "startTransaction"), commitTransaction: /* @__PURE__ */ __name(async () => {
    }, "commitTransaction"), abortTransaction: /* @__PURE__ */ __name(async () => {
    }, "abortTransaction"), endSession: /* @__PURE__ */ __name(() => {
